...
public interface IDAllocMapper {
    ...
    @Update("UPDATE leaf_alloc SET max_id = max_id + step, update_time=GETDATE()  WHERE biz_tag = #{tag}")
    void updateMaxId(@Param("tag") String tag);

    @Update("UPDATE leaf_alloc SET max_id = max_id + #{step} , update_time=GETDATE() WHERE biz_tag = #{key}")
